...
def face_detection_scale(img, model, im_scale):
    img = cv2.resize(img, (0, 0), fx=im_scale, fy=im_scale, interpolation=cv2.INTER_NEAREST)
    img = np.float32(img)
    ...
    im_wid = img.shape[2]
    im_hei = img.shape[1]
    scale = torch.Tensor([im_wid, im_hei, im_wid, im_hei,
                          im_wid, im_hei, im_wid, im_hei,
                          im_wid, im_hei, im_wid, im_hei,
                          im_wid, im_hei])
    ...
    # post processing
    priorbox = Face_Detector.PriorBox(Face_Detector.cfg, image_size=(im_hei, im_wid))
...
if __name__ == '__main__':
    ...
    img_scale = 0.5
    im_width = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH)*img_scale)
    im_height = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT)*img_scale)
    ...
    while True:
        ...
        # det_scale = 0.5
        # dets = face_detection_scale(frame.copy(), net, det_scale)
        ...
        # print(detection)
        rela_bbox = detection.location_data.relative_bounding_box
        xmin, ymin, xw, yh = rela_bbox.xmin, rela_bbox.ymin, rela_bbox.width, rela_bbox.height
        bbox = np.array([xmin, ymin, xmin+xw, ymin+yh*1.1])

        if bbox is not None:
            bbox[0] = int(bbox[0] * im_width + 0.5)
            bbox[2] = int(bbox[2] * im_width + 0.5)
            bbox[1] = int(bbox[1] * im_height + 0.5)
            bbox[3] = int(bbox[3] * im_height + 0.5)
