...
def estimate_transform_pose(pts, in_train=True, crop_size=224):
    ...
    if in_train:
        old_size = old_size * np.random.uniform(1.4, 1.6)
    else:
        old_size = old_size * 1.6
    center = torch.cat(((left + right).unsqueeze(-1) * 0.5, (top + bottom).unsqueeze(-1) * 0.5), dim=-1)
    if in_train:
        center[:, 0] += (torch.rand_like(center[:, 0]) * 2.0 - 1.0) * old_size * 0.15
        center[:, 1] += (torch.rand_like(center[:, 0]) * (-1.0)) * old_size * 0.15
    else:
        center[:, 1] += (-0.25) * old_size * 0.15
    ...
    if in_train:
        ...
        rot_angle = (torch.rand_like(center[:, 0]) * 2.0 - 1.0) * 20.0
...
def estimate_transform_exp(pts, crop_size=224):
    # pts: (b, n, 2)
    b, n = pts.shape[:2]
    left_top = torch.min(pts, dim=1)[0]
    right_bottom = torch.max(pts, dim=1)[0]
    left, right, top, bottom = left_top[..., 0], right_bottom[..., 0], left_top[..., 1], right_bottom[..., 1]
    old_size = (right - left + bottom - top) / 2.0 * 1.1
    center = torch.cat(((left + right).unsqueeze(-1) * 0.5, (top + bottom).unsqueeze(-1) * 0.5), dim=-1)
    old_hsize = old_size.unsqueeze(-1) * 0.5
    src_pts = torch.cat(
        (
    ...
    dst_pts[:, 1, 1] = crop_size - 1.0
    dst_pts[:, 2, 0] = crop_size - 1.0
    return estimate_transform_2d(src_pts, dst_pts)
...
def warp_cams(input_cams, warp_mat):
    ## (b, 4), (b, 2, 3)
    out_cams = torch.zeros_like(input_cams)
    out_cams[:, 0] = input_cams[:, 0] * warp_mat[:, 0, 0]
    out_cams[:, 1] = input_cams[:, 1] * warp_mat[:, 1, 1]
    out_cams[:, 2] = input_cams[:, 2] * warp_mat[:, 0, 0] + warp_mat[:, 0, 2]
    out_cams[:, 3] = input_cams[:, 3] * warp_mat[:, 1, 1] + warp_mat[:, 1, 2]
    return out_cams
